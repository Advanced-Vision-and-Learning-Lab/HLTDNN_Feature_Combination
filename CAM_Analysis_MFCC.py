# -*- coding: utf-8 -*-
"""
Created on Thu Apr  4 23:09:58 2024
...
def main(Params):
    ...
    model_ft.load_state_dict(torch.load('Best_Weights_MFCC.pt'))
    ...
    input_tensor = feature_extraction_layer(sample_signal.unsqueeze(0))  # Shape: [1, 1, H, W]
    # Forward pass 
    ...
    # rgb_img = input_tensor[0].detach().cpu().numpy()  
    ...
    # MFCC-specific labels
    y_axis_label = 'Coefficients'
    x_axis_label = 'Time (s)'
    cmap = 'viridis'  # Choose 'hot' or 'viridis' based on preference for CAM
    
    # Assuming index 1 corresponds to the MFCC channel, if MFCC is in a specific channel position
    i = 1  # Adjust based on your data structure
    
    # Fetch the MFCC data
    original_img = input_tensor[0].detach().cpu().numpy()
    original_img = np.transpose(original_img, (1, 2, 0))
    
    # Normalize the MFCC data for CAM visualization
    ...
    # Displaying the original MFCC data
    im = axs[0].imshow(original_img, aspect='auto')
    axs[0].set_xlabel(x_axis_label, fontsize=14, fontweight='bold')
    axs[0].set_ylabel(y_axis_label, fontsize=14, fontweight='bold')
    axs[0].axis('on')
    
    # Colorbar for the original MFCC, reflecting its actual data range
    cbar = fig.colorbar(im, ax=axs[0], fraction=0.046, pad=0.04)
    cbar.ax.set_ylabel('Intensity', fontsize=12, fontweight='bold')
    
    # Displaying the CAM overlay for MFCC
    cam_image = axs[1].imshow(visualization, cmap=cmap, aspect='auto')
    axs[1].set_xlabel(x_axis_label, fontsize=14, fontweight='bold')
    axs[1].set_ylabel(y_axis_label, fontsize=14, fontweight='bold')
    axs[1].axis('on')
    
    # Colorbar for the CAM overlay, correctly using the data's range
    vmin, vmax = grayscale_cam.min(), grayscale_cam.max()
    norm = plt.Normalize(vmin=vmin, vmax=vmax)
    sm = plt.cm.ScalarMappable(cmap=cmap, norm=norm)
    sm.set_array([])
    cbar_cam = fig.colorbar(sm, ax=axs[1], fraction=0.046, pad=0.04)
    cbar_cam.ax.set_ylabel('Activation', fontsize=12, fontweight='bold')
    
    plt.tight_layout()
    plt.savefig(f'CAM_Figures/MFCC_2_class_{target_class_index}.png', dpi=150)
    plt.close(fig)
    ...
    cbar.ax.set_ylabel('Intensity', fontsize=12, fontweight='bold')
    plt.tight_layout()
    plt.savefig(f'CAM_Figures/Original_MFCC_class_{target_class_index}.png', dpi=150)
    plt.close(fig)
    ...
    cbar_cam.ax.set_ylabel('Activation', fontsize=12, fontweight='bold')
    plt.tight_layout()
    plt.savefig(f'CAM_Figures/CAM_Overlay_MFCC_class_{target_class_index}.png', dpi=150)
    plt.close(fig)
